...
def _with_codes_lock(mutate_fn):
    ...
    with _CODES_LOCK:
        in_mem_res = None
        try:
            in_mem_res = mutate_fn(_IN_MEMORY_CODES)
        except Exception:
            pass
        ...
        try:
            ...
            logger.debug("Code store file notice (in-memory mirrored): %s", e)
            return in_mem_res
...
def _pop_code(key: str):
    ...
    res = _with_codes_lock(mutate)
    if res is None:
        with _CODES_LOCK:
            res = _IN_MEMORY_CODES.pop(key, None)
    return res


def _generate_oauth_state(provider: str) -> str:
    """Generate a cryptographically signed state token with 10-minute validity.
    Works seamlessly across multiple uvicorn workers, containers, and server restarts."""
    random_part = secrets.token_urlsafe(24)
    ts = str(int(time.time()))
    message = f"{provider}:{ts}:{random_part}".encode("utf-8")
    from Auth.Token import ACCESS_SECRET_KEY
    secret = ACCESS_SECRET_KEY or "hrms-sso-state-signing-key"
    sig = hmac.new(secret.encode("utf-8"), message, hashlib.sha256).hexdigest()[:32]
    signed_state = f"{provider}.{ts}.{random_part}.{sig}"
    try:
        _set_code(f"state:{signed_state}", {"provider": provider, "expires": time.time() + 600})
    except Exception:
        pass
    return signed_state
...
def _verify_oauth_state(state: str, expected_provider: str) -> bool:
    """Validate state via local storage pop (replay defense) or cryptographic signature."""
    if not state or not isinstance(state, str):
        return False

    # 1. Check local codes store first
    popped = None
    try:
        _cleanup_codes()
        popped = _pop_code(f"state:{state}")
    except Exception as e:
        logger.warning("Error checking state store: %s", e)

    if popped and popped.get("provider") == expected_provider:
        if popped.get("expires", 0) >= time.time() - 30:
            return True

    # 2. Verify cryptographic signature (ensures state was generated by this backend)
    parts = state.split(".")
    if len(parts) == 4:
        provider, ts_str, random_part, sig = parts
        ...
        now = time.time()
        if now < ts - 60 or now > ts + 600:
            logger.warning("SSO state timestamp expired or outside window: %s", ts)
            return False

        from Auth.Token import ACCESS_SECRET_KEY
        secret = ACCESS_SECRET_KEY or "hrms-sso-state-signing-key"
        message = f"{provider}:{ts_str}:{random_part}".encode("utf-8")
        expected_sig = hmac.new(secret.encode("utf-8"), message, hashlib.sha256).hexdigest()[:32]
...
async def sso_callback_google(code: str = "", state: str = "", error: str = ""):
    ...
    if not _verify_oauth_state(state, "google"):
        logger.warning("Google SSO state validation failed: state=%s", state)
...
async def sso_callback_microsoft(code: str = "", state: str = "", error: str = "", error_description: str = ""):
    try:
        ...
        if not _verify_oauth_state(state, "microsoft"):
            logger.warning("Microsoft SSO state validation failed: state=%s", state)
            return RedirectResponse(f"{FRONTEND_URL}/login?sso_error=invalid_state")

        if not code:
            return RedirectResponse(f"{FRONTEND_URL}/login?sso_error=missing_code")
...
DEFAULT_BOOTSTRAP_ADMIN_EMAILS = {
    "boomika@tibos.in",
    "admin@hrms.com",
    "boomika@m365x02517083.onmicrosoft.com",
}


def _is_bootstrap_admin(email: str) -> bool:
    """Determine if an initial SSO sign-in should receive the admin role.
    Explicitly checks bootstrap admin emails and configured environment admins,
    preventing arbitrary substring-based privilege escalation."""
    email_clean = (email or "").strip().lower()
    if not email_clean:
        return False
    if email_clean in DEFAULT_BOOTSTRAP_ADMIN_EMAILS:
        return True
...
def _finish_sso(email: str, display_name: str = ""):
    ...
    try:
        ...
        if not user:
            ...
            is_admin = _is_bootstrap_admin(email_clean)
